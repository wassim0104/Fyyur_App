...
def venues():
  ...
  for venue in venues:
    list_city_state.append((venue.city, venue.state))

  list_city_state = list(set(list_city_state))
...
def search_venues():
  ...
  # For filtering of result and to be sure it would be case insensitive
  # use methode filter to be able to use ilike method on the name
  venue=Venue.query.filter(Venue.name.ilike('%' + search_term + '%'))
  venue_list=list()
  date = datetime.now()
  # Not necessary, but it is unuseful to show ended shows, for that...
  for v in venue:
    shows = Show.query.filter_by(venue_id=v.id).all()
    upcoming = 0
    for s in shows:
      if s.start_show > date:
        upcoming +=1
...
def show_venue(venue_id):
  ...
  # Get the shows passed, and upcoming
  past_shows=list()
  upcoming_shows = list()
  number_past_shows = 0
  ...
  date = datetime.now()
  if not venue:
    # There is no venue available, return to index page
    ...
    for s in shows:
      ...
      data = {
        ...
        "upcoming_shows_count": number_upcoming_shows,
      }
